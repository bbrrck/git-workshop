...
- Pomocou príkazu `git fetch` stiahneme zoznam *remote* verzií:
```sh
git fetch
```
- Zmeny ktoré nie sú súčasťou lokálneho repozitára zlúčime pomocou `git merge` (bez argumentov):
```sh
git merge
```
- Vo väčšine môžeme jednoducho zavolať príkaz `git pull`, ktorý je kombináciou `git fetch` a `git merge`:
```sh
git pull
```
